 ...
 * module is the single place that parses and re-formats them, so
 * every computation across the package rounds the same way.
 *
...
/** Whether a decimal-string amount is zero (to cent precision). */
...
export function negateAmount(amount: string): string {
  return toMoney(-parseDecimal(amount));
}
