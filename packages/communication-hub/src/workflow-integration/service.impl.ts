 ...
 * notifications, and escalation notifications by composing the
 * Workflow Engine's public runtime API (`defineWorkflow` +
 ...
 * the Communication Hub fully usable offline.
 *
...
import type { CommunicationWorkflowType, WorkflowRequest } from './types.js';
...
type CommunicationWorkflowCategory = 'approval' | 'operational' | 'governance';

interface RequestWorkflowConfig {
  readonly code: string;
  readonly name: string;
  readonly category: CommunicationWorkflowCategory;
}

const REQUEST_WORKFLOW_CONFIG: Readonly<Record<CommunicationWorkflowType, RequestWorkflowConfig>> = {
  approval_reminder: { code: 'communication.approval-reminder', name: 'Approval Reminder', category: 'approval' },
  follow_up_reminder: { code: 'communication.follow-up-reminder', name: 'Follow-up Reminder', category: 'operational' },
  overdue_notification: { code: 'communication.overdue-notification', name: 'Overdue Notification', category: 'operational' },
  escalation_notification: { code: 'communication.escalation-notification', name: 'Escalation Notification', category: 'governance' },
};

export interface GenerateWorkflowRequestInput {
  readonly requestType: CommunicationWorkflowType;
...
export interface WorkflowIntegrationService {
  ...
  listByConversation(organizationId: OrganizationId, conversationId: ConversationId): Promise<readonly WorkflowRequest[]>;
}
...
export function createWorkflowIntegrationService(
  ...
  async function ensureWorkflowDefinitionId(organizationId: OrganizationId, requestType: CommunicationWorkflowType): Promise<string | undefined> {
    ...
    async generateRequest(organizationId, input) {
      ...
      if (deps.workflow && workflowDefinitionId) {
        const instance = await deps.workflow.startWorkflow({
          ...
          variables: { conversationId: input.conversationId, notes: input.notes, dueAt: input.dueAt },
        });
