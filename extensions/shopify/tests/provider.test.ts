...
describe('Shopify Provider', () => {
  it('exports connector manifest', () => {
    expect(connectorManifest.provider).toBe('shopify');
    expect(connectorManifest.id).toBe('shopify-connector');
  });

  it('testConnection returns ok with credentials', async () => {
    const settings = Object.fromEntries(["clientId","clientSecret","shopDomain"].map((k) => [k, 'test-value']));
