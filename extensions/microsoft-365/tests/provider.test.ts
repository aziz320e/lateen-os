...
describe('Microsoft 365 Provider', () => {
  it('exports connector manifest', () => {
    expect(connectorManifest.provider).toBe('microsoft-365');
    expect(connectorManifest.id).toBe('microsoft-365-connector');
  });

  it('testConnection returns ok with credentials', async () => {
    const settings = Object.fromEntries(["clientId","clientSecret","tenantId"].map((k) => [k, 'test-value']));
  ...
  it('sync pull returns records', async () => {
    const settings = Object.fromEntries(["clientId","clientSecret","tenantId"].map((k) => [k, 'test-value']));
    const result = await provider.sync.pull({ settings }, 'email');
