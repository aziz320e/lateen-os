...
describe('Odoo Provider', () => {
  it('exports connector manifest', () => {
    expect(connectorManifest.provider).toBe('odoo');
    expect(connectorManifest.id).toBe('odoo-connector');
  });

  it('testConnection returns ok with credentials', async () => {
    const settings = Object.fromEntries(["baseUrl","apiKey","database"].map((k) => [k, 'test-value']));
