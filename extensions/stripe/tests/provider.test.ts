...
describe('Stripe Provider', () => {
  it('exports connector manifest', () => {
    expect(connectorManifest.provider).toBe('stripe');
    expect(connectorManifest.id).toBe('stripe-connector');
  });

  it('testConnection returns ok with credentials', async () => {
    const settings = Object.fromEntries(["secretKey"].map((k) => [k, 'test-value']));
  ...
  it('sync pull returns records', async () => {
    const settings = Object.fromEntries(["secretKey"].map((k) => [k, 'test-value']));
    const result = await provider.sync.pull({ settings }, 'payments');
