...
describe('WhatsApp Business Provider', () => {
  it('exports connector manifest', () => {
    expect(connectorManifest.provider).toBe('whatsapp-business');
    expect(connectorManifest.id).toBe('whatsapp-business-connector');
  });

  it('testConnection returns ok with credentials', async () => {
    const settings = Object.fromEntries(["accessToken","phoneNumberId"].map((k) => [k, 'test-value']));
  ...
  it('sync pull returns records', async () => {
    const settings = Object.fromEntries(["accessToken","phoneNumberId"].map((k) => [k, 'test-value']));
    const result = await provider.sync.pull({ settings }, 'messages');
