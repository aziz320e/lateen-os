...
describe('Google Drive Provider', () => {
  it('exports connector manifest', () => {
    expect(connectorManifest.provider).toBe('google-drive');
    expect(connectorManifest.id).toBe('google-drive-connector');
  });
  ...
  it('sync pull returns records', async () => {
    const settings = Object.fromEntries(["clientId","clientSecret"].map((k) => [k, 'test-value']));
    const result = await provider.sync.pull({ settings }, 'files');
