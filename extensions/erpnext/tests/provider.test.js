...
describe('ERPNext Provider', () => {
    it('exports connector manifest', () => {
        expect(connectorManifest.provider).toBe('erpnext');
        expect(connectorManifest.id).toBe('erpnext-connector');
    });
    it('testConnection returns ok with credentials', async () => {
        const settings = Object.fromEntries(["baseUrl", "apiKey", "apiSecret"].map((k) => [k, 'test-value']));
    ...
    it('sync pull returns records', async () => {
        const settings = Object.fromEntries(["baseUrl", "apiKey", "apiSecret"].map((k) => [k, 'test-value']));
        const result = await provider.sync.pull({ settings }, 'items');
