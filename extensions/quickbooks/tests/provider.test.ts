...
describe('QuickBooks Provider', () => {
  it('exports connector manifest', () => {
    expect(connectorManifest.provider).toBe('quickbooks');
    expect(connectorManifest.id).toBe('quickbooks-connector');
  });

  it('testConnection returns ok with credentials', async () => {
    const settings = Object.fromEntries(["clientId","clientSecret","realmId"].map((k) => [k, 'test-value']));
  ...
  it('sync pull returns records', async () => {
    const settings = Object.fromEntries(["clientId","clientSecret","realmId"].map((k) => [k, 'test-value']));
    const result = await provider.sync.pull({ settings }, 'invoices');
